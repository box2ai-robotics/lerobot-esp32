...
```bash
box2driver                     # 自动检测串口，启动 Web + STS + 虚拟 COM 桥接
box2driver -p COM5             # 指定串口
box2driver --no-bridge         # 禁用 com0com/socat 虚拟 COM 桥接
...
```
  设备        | MAC             | 端口 (Windows)  | 端口 (Linux/macOS)
  ------------|-----------------|-----------------|-----------------------------
  Leader      | ...2C:3E:28     | COM51           | socket://localhost:6560
  Follower    | ...6D:53:E0     | COM53           | socket://localhost:6561
```

每个设备同时分配 TCP 端口（`socket://localhost:<port>`）和虚拟 COM 端口（需要安装下方的虚拟串口驱动）。TCP 端口始终可用，无需额外安装。

**Windows**: 打开 FD 软件 → 选择 COM 端口（如 COM53）→ 波特率 1000000 → 搜索舵机
**Linux/macOS**: `scservo_sdk` 使用 `PortHandler('socket://localhost:6561')` 或 `PortHandler('/dev/ttyACM0')`

跨平台虚拟串口前置安装（COM 端口映射需要）：
...
**Windows GUI 烧录：Flash Download Tool**

也可以使用图形化工具 `bin/flash_download_tool/flash_download_tool_3.9.9_R2.exe`。

> **驱动安装：** 如果电脑无法识别 ESP32 的 USB 串口，需要先安装 CP210x 驱动。驱动位于 `bin/flash_download_tool/CP210x_USB_TO_UART/`，根据你的 Windows 版本选择对应的安装包（Win10 / Win7-8 64位 / XP-Win7 32位），安装后重新插入 ESP32。

1. 打开工具，选择 **ChipType: ESP32**、**WorkMode: Develop**、**LoadMode: UART**，点击 OK：

   ![Flash Download Tool - 选择 ESP32](assets/flash_esp32.jpg)

2. 添加 3 个 bin 文件并填入对应地址，选择串口号和波特率 921600，点击 **START**：

   ![Flash Download Tool - 选择文件](assets/flash_select_bins.jpg)

3. 等待状态显示 **FINISH / 完成** 即烧录成功：
